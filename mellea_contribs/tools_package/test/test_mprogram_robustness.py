...
A company is ordering catering for 22 people for a Saturday event. The venue is 12 miles away. What is the total cost?"""
    ...
    config.get("target_model", backend_model)
    ...
    def on_progress(current, total, status, entry):
        if status == "baseline":
            ...
            c = G if ok else R
            _out.write(
                f"\r  Baseline: {c}{ans}{X}  |  generating 0/{total} variations..."
            )
            _out.flush()
            return
        elif status == "waiting":
            ...
            _out.write(
                f"\r  [{pct:3d}%] {current}/{total}  {D}{status}: {entry.get('variation_type', '')}{X}   "
            )
            _out.flush()
        else:
            pct = int(current / total * 100)
            ...
            _out.write(
                f"\r  [{pct:3d}%] {current}/{total}  {c}{ans:20s}{X}  {status}   "
            )
            _out.flush()
    ...
    bl_ok = bl["correct"]
    print(
        f"  {B}{'Baseline':30s}{X}  {(G if bl_ok else R)}{bl_ans:20s}{X}  {'PASS' if bl_ok else 'FAIL'}"
    )
    ...
    print(f"{'─' * 70}")
    print(
        f"  Pass rate: {pc}{B}{pr:.0%}{X}  ({report['passed']}/{report['total']})"
...
def parse_args():
    ...
    p.add_argument(
        "--no-enrich", action="store_true", help="Skip LLM feature enrichment (faster)"
    )
    p.add_argument(
        "--judge-model",
        type=str,
