...
def _generate_one_variation(
    gen_client, problem: str, trans_name: str, config: dict, max_retries: int = 2
) -> str:
    system_prompt = (
...
def _validate_variation(
    judge_client, original: str, variation: str, ground_truth: str
) -> bool:
...
def _llm_judge_answer(
    judge_client, problem: str, ground_truth: str, predicted: str
) -> bool:
...
def run_benchdrift_pipeline(
    ...
    m_program_callable: Callable[[str], Any] | None = None,
    mellea_session: MelleaSession | None = None,
    answer_extractor: Callable[[Any], str] | None = None,
    ...
    judge_client = (
        ...
        else ModelClientFactory.create_client(judge_client_type, judge_model_name)
    )
    target_client = (
        None
        if m_program_callable
        else ModelClientFactory.create_client(target_client_type, target_model_name)
    )
    ...
    all_types = {
        k: v
        for k, v in UnifiedVariationEngine.get_all_transformation_types().items()
    ...
    # Baseline
    baseline_answer = _get_answer(
        baseline_problem, m_program_callable, answer_extractor, target_client
    )
    if use_llm_judge:
        baseline_correct = _llm_judge_answer(
            judge_client, baseline_problem, ground_truth_answer, baseline_answer
        )
    else:
    ...
    def _pipeline_worker():
        ...
        for idx, (trans_name, score, axis) in enumerate(valid_ranked):
            ...
            if not skip_validation:
                if not _validate_variation(
                    judge_client, baseline_problem, variation, ground_truth_answer
                ):
    ...
    while True:
        ...
        _idx, trans_name, score, axis, variation, gen_status = item
        processed += 1

        if gen_status == "skip":
            if progress_callback:
                progress_callback(
                    processed, total, "skip", {"variation_type": trans_name}
                )
            continue
        if gen_status == "invalid":
            if progress_callback:
                progress_callback(
                    processed, total, "invalid", {"variation_type": trans_name}
                )
            continue

        # Test immediately via m-program (different model, no contention)
        answer = _get_answer(
            variation, m_program_callable, answer_extractor, target_client
        )

        if use_llm_judge:
            correct = _llm_judge_answer(
                judge_client, variation, ground_truth_answer, answer
            )
        else:
...
def extract_repair_candidates(
    probes: list[dict[str, Any]], baseline_problem: str
) -> list[str]:
